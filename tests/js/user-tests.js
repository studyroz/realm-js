...
module.exports = {
  ...
  async testRemoteMongoClientWatch() {
    ...
    let credentials = Realm.Credentials.anonymous();
    let user = await app.logIn(credentials);
    let collection = user.remoteMongoClient('BackingDB').db('test').collection('testRemoteMongoClient');

    const sleep = async time => new Promise(resolve => setInterval(resolve, time));
    const str = 'use some odd chars to forces weird encoding %\n\r\n\\????>>>>';
    await Promise.all([
      (async () => {
        // There is a race with creating the watch() streams. 
        // Wait 500ms (400+100) before first insert to try to avoid it.
        await sleep(400);
        for (let i = 0; i < 10; i++) {
          await sleep(100);
